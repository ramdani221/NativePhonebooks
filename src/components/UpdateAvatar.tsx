...
export default function UpdateAvatar() {
    ...
    const onButtonPress: Function = useCallback((type: string, options: any) => {
        if (type === 'capture') {
    ...
    return (
        <View style={style.avatarBox}>
            <View style={style.btnSelect}>
                <TouchableOpacity style={style.btnTop} onPress={() => navigation.goBack()}>
            ...
            {showAvt && <Image style={style.img} source={{ uri: `http://192.168.1.50:3000/images/${store.params.avatar ? store.params.avatar : 'user-tie-solid.png'}` }} />}
            {file?.assets && <Image style={style.img} source={{ uri: file?.assets[0].uri }} />}
            <View style={style.btnSelect}>
                <TouchableOpacity style={style.btn} onPress={() => onButtonPress('capture', {
                    saveToPhotos: true,
                    mediaType: 'photo',
                    includeBase64: false,
                },)}>
                ...
                <TouchableOpacity style={style.btn} onPress={() => onButtonPress('library', {
                    selectionLimit: 1,
                    mediaType: 'photo',
                    includeBase64: false,
                },)}>
